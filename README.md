...
1. Configure as credenciais do BigQuery (variável de ambiente `GOOGLE_APPLICATION_CREDENTIALS` apontando para o JSON da service account).
2. Execute as queries em `sql/` para extrair os dados brutos.
3. Rode `notebooks/analise_exploratoria_1746.ipynb` para reproduzir a limpeza e o profiling.
4. Rode `scripts/pipeline_resiliencia_urbana.py` para gerar as figuras e a tabela executiva em `outputs/`.
